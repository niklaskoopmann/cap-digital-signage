...
def prompt_password(key: str, current: str) -> str:
    ...
    We use getpass because Rich's Prompt doesn't mask input by default.
    """
...
def load_config() -> Config:
    ...
    return Config(
        ...
        auth_mode=auth_mode,
        secrets_file=os.getenv("SECRETS_FILE", "secrets.json").strip(),
...
class WifiManager:
    ...
    Why:
    - `netsh wlan connect` works only if a Windows WLAN profile exists.
    - For first-time users (no profile), we create/import a WLAN profile XML
      including SSID + password.
    """
...
"""
    ...
    def disconnect() -> None:
        ...
        if res.returncode != 0:
            logging.warning("Wi-Fi disconnect returned %s.\nSTDOUT:\n%s\nSTDERR:\n%s",
...
# =============================================================================
# Xibo API client (AUTH_MODE=none supported)
# =============================================================================

class XiboClient:
    """
    Minimal Xibo CMS API client.
    ...
    Auth modes:
    - none: no auth header (works only if CMS/API auth disabled)
    - oauth: optional (prepared for later)
    """
    ...
    def authenticate_oauth(self, secrets_path: Path) -> None:
        """OAuth client credentials (prepared for later)."""
        secrets = json.loads(secrets_path.read_text(encoding="utf-8"))
        client_id = secrets.get("client_id")
        client_secret = secrets.get("client_secret")
        if not client_id or not client_secret:
            raise ValueError("secrets.json must contain client_id and client_secret for AUTH_MODE=oauth")

        url = f"{self.base_url}/api/authorize/access_token"
        payload = {
    ...
    def health_check(self) -> None:
        ...
        if r.status_code in (401, 403):
            raise RuntimeError(f"API unauthorized ({r.status_code}). If auth is enabled, switch AUTH_MODE=oauth later.")
        if r.status_code != 200:
    ...
    def upload_media(
        ...
        We use requests-toolbelt MultipartEncoderMonitor to update progress by bytes uploaded.
        """
...
def build_local_index(files: List[Path], mode: str, hash_tag_prefix: str) -> Dict[str, Path]:
    ...
    - filename mode: key = filename
    - hash mode:     key = sha256:<hash>
    """
    idx: Dict[str, Path] = {}
    if mode == "filename":
        for f in files:
...
def build_remote_index(items: List[dict], mode: str, hash_tag_prefix: str) -> Dict[str, dict]:
    ...
    - filename mode: key = item['name'] (fallbacks: fileName/originalFileName)
    - hash mode:     key = tag like sha256:<hash>
    """
    idx: Dict[str, dict] = {}
    for it in items:
        if mode == "filename":
...
def config_wizard(env_path: Path) -> None:
    ...
    env_data["WIFI_PROFILE"] = prompt_edit("WIFI_PROFILE", get("WIFI_PROFILE", env_data["WIFI_SSID"]),
    ...
    env_data["WIFI_AUTH"] = prompt_edit("WIFI_AUTH", get("WIFI_AUTH", "WPA2PSK"),
                                        help_text="WPA2PSK | WPAPSK | open",
                                        validator=must_be_wifi_auth)
    env_data["WIFI_CIPHER"] = prompt_edit("WIFI_CIPHER", get("WIFI_CIPHER", "AES"),
                                          help_text="AES | TKIP",
                                          validator=must_be_wifi_cipher)
    env_data["WIFI_CONNECT_TIMEOUT_SECONDS"] = prompt_edit("WIFI_CONNECT_TIMEOUT_SECONDS",
                                                          get("WIFI_CONNECT_TIMEOUT_SECONDS", "30"),
                                                          help_text="Seconds to wait for Wi-Fi connection.",
                                                          validator=lambda v: int(v))

    # CMS
    env_data["CMS_BASE_URL"] = prompt_edit("CMS_BASE_URL", get("CMS_BASE_URL", "http://192.168.1.1"),
                                          help_text="Xibo CMS base URL on the hotspot network.",
                                          validator=must_be_url)
    env_data["CMS_VERIFY_TLS"] = prompt_edit("CMS_VERIFY_TLS", get("CMS_VERIFY_TLS", "false"),
                                            help_text="Use true only for HTTPS with valid certs.")
    env_data["CMS_TIMEOUT_SECONDS"] = prompt_edit("CMS_TIMEOUT_SECONDS", get("CMS_TIMEOUT_SECONDS", "30"),
                                                  help_text="HTTP timeout in seconds.",
                                                  validator=lambda v: int(v))

    # Auth
    env_data["AUTH_MODE"] = prompt_edit("AUTH_MODE", get("AUTH_MODE", "none"),
                                        help_text="none (no auth header) | oauth (prepared for later)",
                                        validator=must_be_auth)
    env_data["SECRETS_FILE"] = prompt_edit("SECRETS_FILE", get("SECRETS_FILE", "secrets.json"))

    # Local media
    env_data["LOCAL_MEDIA_DIR"] = prompt_edit("LOCAL_MEDIA_DIR", get("LOCAL_MEDIA_DIR", "../images"),
                                              help_text="Relative to scripts/ recommended (../images).")
    env_data["MEDIA_EXTENSIONS"] = prompt_edit("MEDIA_EXTENSIONS",
                                              get("MEDIA_EXTENSIONS", ".jpg,.jpeg,.png,.gif,.mp4,.mov,.mkv,.webm"),
                                              help_text="Comma-separated file extensions to sync.")
    env_data["COMPARE_MODE"] = prompt_edit("COMPARE_MODE", get("COMPARE_MODE", "filename"),
                                          help_text="filename | hash",
                                          validator=must_be_mode)

    # Safety
    env_data["MANAGED_TAG"] = prompt_edit("MANAGED_TAG", get("MANAGED_TAG", "xibo-sync"),
                                          help_text="Tag used for media managed by this script.")
    env_data["ONLY_DELETE_MANAGED_TAG"] = prompt_edit("ONLY_DELETE_MANAGED_TAG",
                                                     get("ONLY_DELETE_MANAGED_TAG", "true"),
                                                     help_text="Recommended true to avoid deleting unrelated media.")
    env_data["MANAGED_FOLDER_ID"] = prompt_edit("MANAGED_FOLDER_ID", get("MANAGED_FOLDER_ID", ""),
    ...
    env_data["DELETE_REMOTE_NOT_LOCAL"] = prompt_edit("DELETE_REMOTE_NOT_LOCAL",
                                                      get("DELETE_REMOTE_NOT_LOCAL", "false"),
                                                      help_text="Default for deletion question per run.")
    env_data["DRY_RUN"] = prompt_edit("DRY_RUN", get("DRY_RUN", "false"),
                                      help_text="true = preview only (no upload/delete)")

    # Refresh
    env_data["DISPLAY_GROUP_ID"] = prompt_edit("DISPLAY_GROUP_ID", get("DISPLAY_GROUP_ID", ""),
                                               help_text="Optional displayGroupId for Collect Now.")
    env_data["TRIGGER_COLLECTNOW_ON_CHANGES"] = prompt_edit("TRIGGER_COLLECTNOW_ON_CHANGES",
                                                           get("TRIGGER_COLLECTNOW_ON_CHANGES", "true"))

    # Upload / hash
    env_data["XIBO_UPLOAD_FIELD"] = prompt_edit("XIBO_UPLOAD_FIELD", get("XIBO_UPLOAD_FIELD", "files"),
                                                help_text="Multipart field name; script tries fallbacks too.")
    env_data["HASH_TAG_PREFIX"] = prompt_edit("HASH_TAG_PREFIX", get("HASH_TAG_PREFIX", "sha256:"))

    # Logging
    env_data["LOG_LEVEL"] = prompt_edit("LOG_LEVEL", get("LOG_LEVEL", "INFO"))
    env_data["LOG_FILE"] = prompt_edit("LOG_FILE", get("LOG_FILE", "logs/sync_xibo.log"),
...
def main() -> int:
    ...
    override = Confirm.ask(
        ...
        default=(not env_data)  # if no .env, default to running the wizard
    )
    if override:
    ...
    # ✅ Always persist delete choice to .env (requested)
    ...
    try:
        ...
        # Prepared for later: OAuth (if you enable it)
        if cfg.auth_mode == "oauth":
            secrets_path = (scripts_dir / cfg.secrets_file).resolve() if not Path(cfg.secrets_file).is_absolute() else Path(cfg.secrets_file)
            xibo.authenticate_oauth(secrets_path)
